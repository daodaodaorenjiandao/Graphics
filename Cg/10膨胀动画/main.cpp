...
static void createCgPrograms()
{
	...
	g_cgFrequency = cgGetNamedParameter(g_cgVertexProgram, "frequency");
	...
	assert(g_cgWorldMatrix && g_cgWorldViewProjMatrix  && g_cgAmbient &&
		   g_cgScale && g_cgFrequency && g_cgTime && 
								   ...
								   void* userContext)
{
	...
	if (SUCCEEDED(pDev->BeginScene())) {
		...
		cgSetParameter1f(g_cgScale, 0.1);
		cgSetParameter1f(g_cgFrequency, 10);
