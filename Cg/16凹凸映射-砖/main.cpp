...
static void createCgPrograms()
{
	...
	assert(g_cgWorldViewProj && g_cgLightPos);
	...
	assert(g_cgNormalTexture && g_cgCubeTexture);
}
...
static HRESULT initVertexBuffer(IDirect3DDevice9* pDev)
{
	...
	*pVertex = Vertex(-1, 1, 0, 0, 0);
	*(pVertex + 1) = Vertex(1, 1, 0, 1, 0);
	*(pVertex + 2) = Vertex(-1, -1, 0, 0, 1);
	*(pVertex + 3) = Vertex(1, -1, 0, 1, 1);
...
HRESULT init(IDirect3DDevice9 * pDevice)
{
	...
	D3DXVECTOR3 pos(0.f, 0.f, -5.f);		//原先这个z值为-1，而显示的图形在z=0平面上，近裁剪面为z=1，所以图像不能显示出来
	...
	//hr = D3DXCreateTextureFromFile(pDevice, _T("brick.jpg"), &g_pNormalTexture);
