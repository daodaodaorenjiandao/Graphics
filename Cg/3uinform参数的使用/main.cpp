...
static CGparameter g_cgColorParam;		//顶点程序的颜色参数  uniform类型
									  ...
									  void* userContext)
{
	...
	if (bFirst)
	{
		...
		g_cgColorParam = cgGetNamedParameter(g_cgVertexProgram, "color");
		checkForCgError("get unifor param color fail");
	}
								   ...
								   void* userContext)
{
	...
	if (SUCCEEDED(pDev->BeginScene()))
	{
		...
		cgSetParameter4f(g_cgColorParam, 1, 0, 0, 1);	//设置uniform参数
