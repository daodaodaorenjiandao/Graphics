...
static void createCgPrograms()
{
	...
	g_cgWorldViewProj = cgGetNamedParameter(g_cgVertexProgram, "modelViewProj");
	g_cgLightPos = cgGetNamedParameter(g_cgVertexProgram, "lightPosition");
	...
	g_cgNormalTexture = cgGetNamedParameter(g_cgPixelProgram, "normalMap");
	g_cgCubeTexture = cgGetNamedParameter(g_cgPixelProgram, "normalizeCube");
	//g_cgPower = cgGetNamedParameter(g_cgPixelProgram, "power");
	assert(g_cgNormalTexture && g_cgCubeTexture /*&& g_cgPower*/);
}

static HRESULT initVertexBuffer(IDirect3DDevice9* pDev, int sides, int rings)
{
	...
	hr = pDev->CreateVertexBuffer(numVertsPerPatch * sizeof(Vertex),
								  0, Vertex::FVF,
								  D3DPOOL_DEFAULT,
	...
	for (int i = 0; i < rings; ++i) 
	{
		...
		{
			pVertices[index]._x = i*m;
			pVertices[index]._y = j*n;
			pVertices[index]._z = 0;
			index++;
			pVertices[index]._x = (i + 1)*m;
			pVertices[index]._y = j*n;
			pVertices[index]._z = 0;
			index++;
		}
...
HRESULT init(IDirect3DDevice9 * pDevice)
{
	...
	D3DXVECTOR3 pos(0.f, 0.f, 20.f);		//注意裁剪平面！！否则物体被裁剪不能显示
	...
	return initVertexBuffer(pDevice,20,40);
}
...
HRESULT drawFlatPatch(IDirect3DDevice9* pDev, IDirect3DVertexBuffer9 *vb, int sides, int rings)
{
	...
	hr = pDev->SetStreamSource(0, vb, 0, sizeof(Vertex));
	if (FAILED(hr))
		return hr;
...
#endif
		drawFlatPatch(pDev, g_pVertexBuffer, 20, 40);
