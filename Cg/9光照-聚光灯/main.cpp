...
CGparameter g_cgLightDirection;
CGparameter g_cgLightInnerCone;
CGparameter g_cgLightOuterCone;
...
static void createCgPrograms()
{
	...
	g_cgLightDirection = cgGetNamedParameter(g_cgPixelProgram, "lightDirectional");
	g_cgLightInnerCone = cgGetNamedParameter(g_cgPixelProgram, "innerCone");
	g_cgLightOuterCone = cgGetNamedParameter(g_cgPixelProgram, "outerCone");
	...
	assert(g_cgWorldMatrix && g_cgWorldViewProjMatrix && g_cgAmbient &&
		   ...
		   g_cgLightDirection && g_cgLightInnerCone && g_cgLightOuterCone &&
...
bool init(IDirect3DDevice9 * pDevice)
{
	...
	g_light.Position = D3DXVECTOR3(-0.f, 0.1f, -5.f);
	g_light.Direction = D3DXVECTOR3(0.f, 0.f, 1.f);
	g_light.Theta = 5.f / 360 * D3DX_PI;		//10 
	g_light.Phi = 20.f / 360 * D3DX_PI;			//20
								   ...
								   void* userContext)
{
	...
	if (SUCCEEDED(pDev->BeginScene())) {
		...
		cgSetParameterValuefc(g_cgLightDirection, 3, (float*)&g_light.Direction);
		cgSetParameter1f(g_cgLightInnerCone, g_light.Theta);
		cgSetParameter1f(g_cgLightOuterCone, g_light.Phi);
