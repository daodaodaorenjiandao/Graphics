...
ID3DXMesh * g_pBoxMesh = nullptr;
ID3DXMesh * g_pSphereMesh = nullptr;

ID3DXFont * g_pFont = nullptr;

ID3DXMesh * CreateBoundingBox(ID3DXBaseMesh * pMesh);	//创建网格数据外接体
ID3DXMesh * CreateSphere(ID3DXBaseMesh * pMesh);		//创建网格数据的外接球
void RecreateBoundingBoxAndSphere(ID3DXBaseMesh * pMesh);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		if (wParam == VK_SPACE)
			g_bBoundingBox = !g_bBoundingBox;
		else if (wParam == VK_DOWN)
			YELLOW_MATERIAL.Diffuse.a -= 0.1;
...
BOOL Setup()
{
	...
	//创建外接体和外接球 
	g_pBoxMesh = CreateBoundingBox(g_pPMesh);
	g_pSphereMesh = CreateSphere(g_pPMesh);
...
void Render()
{
	...
	if (::GetAsyncKeyState('A') & 0x8000f)
	{
		...
		RecreateBoundingBoxAndSphere(g_pPMesh);
	}
	...
	g_pDevice->SetRenderState(D3DRS_FILLMODE, D3DFILL_SOLID);
	//g_pDevice->SetMaterial(&)
	g_pDevice->SetRenderState(D3DRS_ALPHABLENDENABLE, TRUE);		//开启alpha融合
	g_pDevice->SetRenderState(D3DRS_SRCBLEND, D3DBLEND_SRCALPHA);
	g_pDevice->SetRenderState(D3DRS_DESTBLEND, D3DBLEND_INVSRCALPHA);
	//YELLOW_MATERIAL.Diffuse.a = 0.1;		//设置10%的opcity 通过按键设置
	g_pDevice->SetMaterial(&YELLOW_MATERIAL);
	...
	g_pDevice->SetRenderState(D3DRS_ALPHABLENDENABLE, TRUE);		//关闭alpha融合
...
ID3DXMesh * CreateBoundingBox(ID3DXBaseMesh * pMesh)
{
	...
	D3DXComputeBoundingBox((const D3DXVECTOR3*)pVertex,
						   pMesh->GetNumVertices(),
						   pMesh->GetNumBytesPerVertex(),
						   &minPos,
						   &maxPos);

	ID3DXMesh * pBoundingBoxMesh = nullptr;
	HRESULT hr;
	hr = D3DXCreateBox(g_pDevice,
					   maxPos.x - minPos.x,
					   maxPos.y - minPos.y,
					   maxPos.z - minPos.z,
					   &pBoundingBoxMesh,
	...
	if (FALSE(hr))
	{
		MessageBox(NULL, _T("创建外接体失败！"), NULL, MB_OK);
		return nullptr;
	}
	return pBoundingBoxMesh;
}

ID3DXMesh * CreateSphere(ID3DXBaseMesh * pMesh)
{
	assert(pMesh != nullptr);
	...
	pVertexBuffer->Lock(0, 0, &pVertex, 0);
	pVertexBuffer->Unlock();
	D3DXComputeBoundingSphere((const D3DXVECTOR3*)pVertex,
							  pMesh->GetNumVertices(),
							  pMesh->GetNumBytesPerVertex(),
							  &posCenter,
							  &radius);
	HRESULT hr;
	ID3DXMesh * pSphereMesh = nullptr;
	hr = D3DXCreateSphere(g_pDevice, radius, 20, 20, &pSphereMesh, NULL);
	if (FAILED(hr))
	{
		MessageBox(NULL, _T("创建外接球失败！"), NULL, MB_OK);
		return nullptr;
	}
	return pSphereMesh;
}
...
void RecreateBoundingBoxAndSphere(ID3DXBaseMesh * pMesh)
{
	SAFE_RELEASE(g_pBoxMesh);
	SAFE_RELEASE(g_pSphereMesh);
	g_pBoxMesh = CreateBoundingBox(pMesh);
	g_pSphereMesh = CreateSphere(pMesh);
}